...
def validate_execution_time(execution_time):
    ...
    total_seconds = (
        execution_time.hour * 3600 + execution_time.minute * 60 + execution_time.second
    )
...
def validate_pleasant_habit(attrs):
    ...
    if reward_action and (addition_habit or award):
        raise ValidationError(
            "У приятной привычки не может быть связанной привычки или награды."
        )
...
def validate_no_both_addition_and_award(attrs):
    ...
    if addition_habit and award:
        raise ValidationError(
            "У привычки не может быть одновременно связанной привычки и награды."
        )
